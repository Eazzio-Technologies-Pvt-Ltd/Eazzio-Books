/**
 * PaymentsReceived.js – Redesigned Payments List UI matching Invoices/Quotes/SalesOrders UI
 */
...
function PaymentsReceived() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(
    ALL_COLUMNS.reduce((acc, col) => {
      acc[col.key] = true;
      if (typeof window !== 'undefined' && window.innerWidth < 768 && ['reference', 'unused_amount'].includes(col.key)) {
        acc[col.key] = false;
      }
      return acc;
    }, {})
  ...
  const filteredPayments = payments.filter((p) => {
    ...
    const matchSearch = search === "" || custName.includes(term) || invNum.includes(term) || ref.includes(term);
    const matchStatus = statusFilter === "all" || (p.status || "completed").toLowerCase() === statusFilter;
  ...
  const handleDeleteSelected = async () => {
    if (!window.confirm(`Delete the ${selectedIds.length} selected Payment(s)?`)) return;
    toast.error("Delete endpoint not configured for payments yet.");
    // try {
    //   await Promise.all(
    //     selectedIds.map((id) => apiRequest(`/payments/${id}`, { method: "DELETE" }))
    //   );
  ...
  const getFilterLabel = (filter) => {
    switch (filter) {
      case "all": return "All Received Payments";
      case "draft": return "Draft Payments";
      case "completed": return "Completed Payments";
      case "failed": return "Failed Payments";
      default: return "All Received Payments";
    }
  };
      ...
      <div className="full-table-container">
        ...
        <div className="full-table-header">
                  ...
                  filteredViews.map(view => {
                    ...
                    return (
                      <div key={view.key} className="view-dropdown-item" onClick={() => { setStatusFilter(view.key); setStatusDropdownOpen(false); }} style={{ background: isSelected ? "#f0f6ff" : "transparent", color: isSelected ? "#006ee6" : "#344054", fontWeight: isSelected ? "500" : "400" }}>
                        ...
                        <span onClick={(e) => { e.stopPropagation(); const newFav = isFav ? null : view.key; setFavoriteView(newFav); if (newFav) { localStorage.setItem("favPaymentView", newFav); toast.success(`"${view.label}" set as default view`); } else { localStorage.removeItem("favPaymentView"); toast.success("Default view cleared"); } }} style={{ color: isFav ? "#f59e0b" : "#d0d5dd", fontSize: "14px", padding: "2px 6px" }}>{isFav ? "★" : "☆"}</span>
                      </div>
                    );
                  })
                )}
          ...
          <div className="table-actions">
            <button className="btn-new" onClick={() => navigate("/payments-received/new")}>
        ...
        {selectedIds.length > 0 && (
          <div style={{ background: "#f0f6ff", border: "1px solid #bae6fd", borderRadius: "8px", padding: "12px 16px", display: "flex", alignItems: "center", gap: "16px", margin: "20px 30px 0" }}>
            <span style={{ color: "#0369a1", fontWeight: "600", fontSize: "13px" }}>{selectedIds.length} payment(s) selected</span>
              ...
              <thead>
                <tr>
                  ...
                  <th style={{ width: '40px', textAlign: 'center', resize: 'none' }}>
                    <input type="checkbox" style={{ accentColor: '#4a90e2', margin: 0 }} checked={selectedIds.length === sortedPayments.length && sortedPayments.length > 0} onChange={handleSelectAll} />
                  </th>
                  {ALL_COLUMNS.filter(c => c.key !== "checkbox" && visibleColumns[c.key]).map(col => (
                    <th key={col.key} style={['amount', 'unused_amount'].includes(col.key) ? { textAlign: 'right' } : {}}>{col.label.toUpperCase()}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {sortedPayments.map((payment) => {
                  ...
                  <tr key={payment.id} onClick={() => navigate(`/payments-received/${payment.id}`)} style={{ cursor: "pointer", background: selectedIds.includes(payment.id) ? "#fcfcfd" : "" }}>
                    ...
                    {visibleColumns.payment_number && <td className="customer-name-link">{payment.id || "—"}</td>}
                    ...
                    {visibleColumns.invoice_number && <td className="customer-name-link">{payment.invoice_number || "—"}</td>}
                    {visibleColumns.payment_mode && <td style={{ textTransform: "capitalize" }}>{payment.payment_mode || "Cash"}</td>}
                    {visibleColumns.amount && <td style={{ textAlign: 'right', fontWeight: "600", color: "#1d2939" }}>₹{parseFloat(payment.amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
                    {visibleColumns.unused_amount && <td style={{ textAlign: 'right', color: "#667085" }}>₹0.00</td>}
