/**
 * Expenses.js – Final version with activity log timeline, active/inactive toggle,
...
function Expenses() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(
    ALL_COLUMNS.reduce((acc, col) => {
      acc[col.key] = true;
      if (typeof window !== 'undefined' && window.innerWidth < 768 && ['category', 'vendor_name'].includes(col.key)) {
        acc[col.key] = false;
      }
      return acc;
    }, {})
  ...
  const fetchExpenses = useCallback(async () => {
    try {
      ...
      if (res) setExpenses(res.expenses || []);
    } catch (err) {
  ...
  const fetchInvoiceData = async (expenseId) => {
    setInvoicesLoading(true);
    try {
      const res = await apiRequest(`/expenses/${expenseId}/invoices`);
  ...
  const fetchComments = async (expenseId) => {
    ...
    try {
      const res = await apiRequest(`/expenses/${expenseId}/comments`);
  ...
  const fetchActivities = async (expenseId) => {
    try {
      const res = await apiRequest(`/expenses/${expenseId}/activity`);
  ...
  const toggleExpand = async (id) => {
    ...
    try {
      ...
      if (res) {
        setExpandedExpense(res.expense);
        setExpandedAddresses(res.addresses || []);
        setExpandedContacts(res.contacts || []);
        setActiveStatus(res.expense.is_active);
        fetchInvoiceData(id);
        fetchComments(id);
        fetchActivities(id);
      }
  ...
  const toggleSelectAll = () => {
    ...
    else setSelected(expenses.map((c) => c.id));
  };
  ...
  const handleInvitePortal = async (expenseId) => {
    try {
  ...
  const handleNewTransaction = (expenseId) => {
  ...
  const handleGenerateStatement = async () => {
    ...
    try {
      ...
      const openingBalance = parseFloat(expandedExpense?.opening_balance) || 0;
  ...
  const openStatementWindow = () => {
    ...
    <style>
      ...
      .to-section .expense-name { color: #2275d7; font-weight: bold; font-size: 15px; }
      ...
      <div class="mid-section">
        <div class="to-section">
          ...
          <div class="expense-name">${getExpenseName(expandedExpense)}</div>
        </div>
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const range = getStatementDates();
      const url = `${process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}/expenses/${expandedId}/statement/pdf?from=${range.from}&to=${range.to}`;
      const response = await fetch(url, { credentials: 'include' });
      if (!response.ok) throw new Error('PDF generation failed');
      const blob = await response.blob();
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Statement_${getExpenseName(expandedExpense)}_${range.from}_${range.to}.pdf`;
  ...
  const openEmailModal = () => {
    if (!generatedStatement) { toast.error('Please generate the statement first.'); return; }
    const range = getStatementDates();
    setEmailTo(expandedExpense.email || '');
    setEmailSubject(`Statement of Accounts – ${getExpenseName(expandedExpense)} (${range.from} to ${range.to})`);
    setEmailBody(`Dear ${getExpenseName(expandedExpense)},\n\nPlease find your Statement of Accounts attached for the period ${range.from} to ${range.to}.\n\nKindly review and let us know if you have any queries.\n\nRegards,\nTinplate Computer Training Center`);
  ...
  const getCondensedMessage = () => {
    const custName = getExpenseName(expandedExpense);
    const range = getStatementDates();
    const balanceDue = parseFloat(generatedStatement?.balanceDue || 0).toFixed(2);
    return `Dear ${custName}, please find your Statement of Accounts from ${range.from} to ${range.to}. Balance Due: ₹${balanceDue}. Thank you. Regards, Tinplate Computer Training Center.`;
  };
  ...
  const sendWhatsApp = async (e) => {
    ...
    let phoneVal = "";
    const loadId = toast.loading("Fetching contact phone...");
    try {
      if (expandedExpense.vendor_id) {
        const res = await apiRequest(`/vendors/${expandedExpense.vendor_id}`);
        phoneVal = res?.vendor?.mobile || res?.vendor?.phone || res?.vendor?.work_phone;
      } else if (expandedExpense.customer_id) {
        const res = await apiRequest(`/customers/${expandedExpense.customer_id}`);
        phoneVal = res?.customer?.mobile || res?.customer?.phone || res?.customer?.work_phone;
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}/expenses/${expandedId}/statement/send`,
        {
          method: 'POST',
          credentials: 'include',
  ...
  const handleDownloadXLS = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `Statement_${getExpenseName(expandedExpense)}_${generatedStatement.from}_${generatedStatement.to}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const addComment = async () => {
    ...
    try {
      const res = await apiRequest(`/expenses/${expandedId}/comments`, {
        method: "POST",
  ...
  const openingBalance = expandedExpense ? parseFloat(expandedExpense.opening_balance) || 0 : 0;
          ...
          <div className="expenses-left-pane" style={{ width: "320px", minWidth: "320px", borderRight: "1px solid #eaecf0", background: "#ffffff", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "16px", borderBottom: "1px solid #eaecf0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                {canAccess(user?.role, MODULES.EXPENSES, ACTIONS.CREATE) && (
                  <button 
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {filteredExpenses.length === 0 ? (
                <div style={{ padding: "40px 16px", textAlign: "center", color: "#667085", fontSize: "13px" }}>No expenses found.</div>
              ) : (
                filteredExpenses.map((c) => {
                  const initials = getExpenseName(c).split(" ").map(w => w[0]).filter(Boolean).slice(0, 2).join("").toUpperCase();
            ...
            expandedLoading ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#ffffff" }}>
                ...
                <div className="detail-header-row" style={{ padding: "16px 24px", borderBottom: "1px solid #eaecf0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#ffffff" }}>
                  ...
                  {/* Header Actions */}
                  <div className="expenses-header-actions" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <button 
                      onClick={() => navigate(`/expenses/${expandedExpense.id}/edit`)} 
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      {newTransactionOpen && (
                        <div style={{ position: "absolute", right: 0, top: "100%", marginTop: "6px", background: "#ffffff", border: "1px solid #eaecf0", borderRadius: "8px", boxShadow: "0 10px 25px rgba(0,0,0,0.08)", zIndex: 100, minWidth: "160px" }}>
                          <button style={menuItem} onClick={() => { setNewTransactionOpen(false); handleNewTransaction(expandedExpense.id); }}>📄 Invoice</button>
                  ...
                  {activeTab === "Overview" && (
                    <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "32px", alignItems: "start" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                        ...
                        <div>
                          <div style={{ fontSize: "12px", color: "#667085", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "4px" }}>Expense Category</div>
                          <div style={{ fontSize: "16px", fontWeight: "600", color: "#1d2939" }}>{expandedExpense.category || expandedExpense.expense_account || "—"}</div>
                        </div>

                        {/* Expense details card */}
                        <div style={{ border: "1px solid #eaecf0", borderRadius: "10px", padding: "20px", display: "flex", gap: "16px", background: "#fcfcfd" }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: "14px", fontWeight: "600", color: "#1d2939", marginBottom: "4px" }}>Amount: ₹{expandedExpense.amount ? parseFloat(expandedExpense.amount).toFixed(2) : "0.00"}</div>
                            <div style={{ fontSize: "13px", color: "#475569", display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
                              <span>📅</span>
                              <span>Date: {expandedExpense.expense_date ? new Date(expandedExpense.expense_date).toLocaleDateString() : "—"}</span>
                            </div>
                            <div style={{ fontSize: "13px", color: "#475569", display: "flex", alignItems: "center", gap: "6px" }}>
                              <span>🏢</span>
                              <span>Vendor: {expandedExpense.vendor_name || "—"}</span>
                            </div>
                            {expandedExpense.customer_name && (
                              <div style={{ fontSize: "13px", color: "#475569", display: "flex", alignItems: "center", gap: "6px", marginTop: "4px" }}>
                                <span>👤</span>
                                <span>Customer: {expandedExpense.customer_name}</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          <h4 style={{ margin: "0 0 12px 0", fontSize: "13px", color: "#475569", textTransform: "uppercase", letterSpacing: "0.03em", fontWeight: "600" }}>Details</h4>
                      ...
                      {generatedStatement && (
                        <div style={{ border: "1px solid #eaecf0", borderRadius: "10px", padding: "24px", background: "#ffffff", boxShadow: "0 4px 15px rgba(16,24,40,0.02)" }}>
                          ...
                          <div style={{ marginBottom: "24px" }}>
                            ...
                            <div style={{ fontSize: "13px", color: "#475569" }}>To: <strong style={{ color: "#1d2939" }}>{getExpenseName(expandedExpense)}</strong></div>
                          ...
                          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                            ...
                            <button onClick={sendWhatsApp} style={{ padding: "8px 16px", background: "#ffffff", border: "1px solid #d0d5dd", color: "#344054", borderRadius: "6px", fontSize: "13px", cursor: "pointer", fontWeight: "600", display: "inline-flex", alignItems: "center", gap: "4px" }} title="Send WhatsApp">
                            ...
                            <button onClick={sendSMS} style={{ padding: "8px 16px", background: "#ffffff", border: "1px solid #d0d5dd", color: "#344054", borderRadius: "6px", fontSize: "13px", cursor: "pointer", fontWeight: "600", display: "inline-flex", alignItems: "center", gap: "4px" }} title="Send SMS">
              ...
              <div style={{ padding: "40px", textAlign: "center" }}>Expense not found.</div>
            )
          ) : (
            // ==================== FULL LIST VIEW MODE ====================
            <div className="full-table-container">
              ...
              <div className="full-table-header">
                ...
                <div className="table-actions">
                  {canAccess(user?.role, MODULES.EXPENSES, ACTIONS.CREATE) && (
                    <button className="btn-new" onClick={() => navigate("/expenses/new")}>
                    ...
                    {showMoreMenu && (
                      <div className="more-dropdown-menu">
                        ...
                        <div className="more-dropdown-item">
                          ...
                          <svg className="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                          <div className="nested-dropdown-menu">
                            <div className="more-dropdown-item"><span>Export Expenses</span></div>
                        ...
                        <div className="more-dropdown-divider"></div>
                        <div className="more-dropdown-item" onClick={() => { fetchExpenses(); setShowMoreMenu(false); }}>
              ...
              {selected.length > 0 && (
                <div style={{ background: "#f0f6ff", border: "1px solid #bae6fd", borderRadius: "8px", padding: "12px 16px", display: "flex", alignItems: "center", gap: "16px", marginBottom: "20px" }}>
                  <span style={{ color: "#0369a1", fontWeight: "600", fontSize: "13px" }}>{selected.length} expense(s) selected</span>
              ...
              <div className="table-wrapper">
                {loading ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '80px 20px', color: '#98a2b3' }}>
                    ...
                    <h3 style={{ color: '#1d2939', marginBottom: '8px', fontSize: "16px", fontWeight: "600" }}>No expenses found</h3>
                    <p style={{ marginBottom: '20px', fontSize: "13px" }}>{searchQuery ? 'No expenses match your search.' : 'Start by adding your first expense.'}</p>
                    {!searchQuery && canAccess(user?.role, MODULES.EXPENSES, ACTIONS.CREATE) && (
                      <button className="btn-new" onClick={() => navigate('/expenses/new')} style={{ margin: "0 auto" }}>+ New Expense</button>
                    )}
                  </div>
                ) : (
                    ...
                    <thead>
                      <tr>
                        ...
                        <th style={{ width: '40px', textAlign: 'center', resize: 'none' }}>
                          <input type="checkbox" style={{ accentColor: '#4a90e2', margin: 0 }} checked={selected.length === filteredExpenses.length && filteredExpenses.length > 0} onChange={toggleSelectAll} />
                        </th>
                      ...
                      {filteredExpenses.map((c) => (
                        <tr key={c.id}>
                          ...
                          {visibleColumns.expense_date && (
                            <td className="expense-name-link" onClick={() => toggleExpand(c.id)}>
                              {c.expense_date ? new Date(c.expense_date).toLocaleDateString() : "—"}
                            </td>
                          )}
                          {visibleColumns.category && <td>{c.category || c.expense_account || "—"}</td>}
                          {visibleColumns.vendor_name && <td>{c.vendor_name || "—"}</td>}
                          {visibleColumns.amount && <td>₹{c.amount ? parseFloat(c.amount).toFixed(2) : "0.00"}</td>}
                          {visibleColumns.status && (
                            <td>
                              <span style={{
                                padding: '3px 8px', borderRadius: '12px', fontSize: '11px', fontWeight: 'bold',
                                background: c.status?.toLowerCase() === 'billed' ? '#dcfce7' : '#f1f5f9',
                                color: c.status?.toLowerCase() === 'billed' ? '#166534' : '#475569'
                              }}>
                                {c.status || "Unbilled"}
                              </span>
                            </td>
