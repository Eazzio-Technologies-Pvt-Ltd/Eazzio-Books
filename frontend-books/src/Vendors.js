...
function Vendors() {
  ...
  const getCondensedMessage = () => {
    const vendName = getVendorName(expandedVendor);
    const range = getStatementDates();
    const balanceDue = parseFloat(generatedStatement?.balanceDue || 0).toFixed(2);
    return `Dear ${vendName}, please find your Statement of Accounts from ${range.from} to ${range.to}. Balance Due: ₹${balanceDue}. Thank you. Regards, Tinplate Computer Training Center.`;
  };

  const sendWhatsApp = (e) => {
    if (e && e.stopPropagation) e.stopPropagation();
    if (!generatedStatement) { toast.error('Please generate the statement first.'); return; }
    const phoneVal = expandedVendor?.mobile || expandedVendor?.phone || expandedVendor?.work_phone;
