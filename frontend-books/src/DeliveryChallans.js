/**
 * DeliveryChallans.js – Redesigned list view matching Quotes UI
 */
...
const ALL_COLUMNS = [
  ...
  { key: "delivery_challan_number", label: "Challan Number" },
...
function DeliveryChallans() {
  ...
  const [favoriteView, setFavoriteView] = useState(() => localStorage.getItem("favDeliveryChallansView") || null);
  ...
  const [sortBy, setSortBy] = useState("challan_date");
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setChallans(Array.isArray(dataRes?.delivery_challans) ? dataRes.delivery_challans : []);
  ...
  useEffect(() => {
    const fav = localStorage.getItem("favDeliveryChallansView");
    if (fav) {
      setStatusFilter(fav);
    }
  }, []);
  ...
  const filteredData = challans.filter((q) => {
    const searchTerms = [
      q.delivery_challan_number || "",
      q.reference_number || "",
      q.profile_name || "",
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        selectedIds.map((id) => apiRequest(`/delivery-challans/${id}`, { method: "DELETE" }))
      );
      ...
      <div className="full-table-container">
        <div className="full-table-header">
                  ...
                  filteredViews.map(view => {
                    ...
                    return (
                      <div key={view.key} className="view-dropdown-item" onClick={() => { setStatusFilter(view.key); setStatusDropdownOpen(false); }} style={{ background: isSelected ? "#f0f6ff" : "transparent", color: isSelected ? "#006ee6" : "#344054", fontWeight: isSelected ? "500" : "400" }}>
                        ...
                        <span onClick={(e) => { e.stopPropagation(); const newFav = isFav ? null : view.key; setFavoriteView(newFav); if (newFav) { localStorage.setItem("favDeliveryChallansView", newFav); toast.success(`"${view.label}" set as default view`); } else { localStorage.removeItem("favDeliveryChallansView"); toast.success("Default view cleared"); } }} style={{ color: isFav ? "#f59e0b" : "#d0d5dd", fontSize: "14px", padding: "2px 6px" }}>{isFav ? "★" : "☆"}</span>
                      </div>
                    );
                  })
                )}
          ...
          <div className="table-actions">
            <button className="btn-new" onClick={() => navigate("/delivery-challans/new")}>
                ...
                <div className="more-dropdown-menu">
                  <div className="more-dropdown-item">
                    ...
                    <div className="nested-dropdown-menu">
                                            <div className="more-dropdown-item" onClick={() => { setSortBy("challan_date"); setSortOrder("desc"); setMoreMenuOpen(false); }}><span>Date (Newest first)</span></div>
                      <div className="more-dropdown-item" onClick={() => { setSortBy("delivery_challan_number"); setSortOrder("asc"); setMoreMenuOpen(false); }}><span>Challan Number (A-Z)</span></div>
                  ...
                  <div className="more-dropdown-item" onClick={() => { setMoreMenuOpen(false); toast("Import functionality coming soon"); }}>
                    ...
                    <span>Import DeliveryChallans</span>
                  </div>
                  <div className="more-dropdown-item">
                    ...
                    <span>Export DeliveryChallans</span>
                    <svg className="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                    <div className="nested-dropdown-menu">
        ...
        <div className="table-wrapper">
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '80px 20px', color: '#98a2b3' }}>
              ...
              <h3 style={{ color: '#1d2939', marginBottom: '8px', fontSize: "16px", fontWeight: "600" }}>No Delivery Challan found</h3>
              <p style={{ marginBottom: '20px', fontSize: "13px" }}>{search ? 'No items match your search.' : 'Start by creating your first challan.'}</p>
              <button className="btn-new" onClick={() => navigate('/delivery-challans/new')} style={{ margin: "0 auto" }}>+ New Challan</button>
            </div>
          ) : (
            <table className={`items-table ${clipText ? 'clip-text' : ''}`}>
              ...
              <tbody>
                {sortedData.map((q) => (
                  <tr key={q.id} onClick={() => navigate(`/delivery-challans/${q.id}`)} style={{ cursor: "pointer", background: selectedIds.includes(q.id) ? "#fcfcfd" : "" }}>
                    ...
                    {visibleColumns.challan_date && <td>{new Date(q.challan_date).toLocaleDateString("en-GB")}</td>}
                    {visibleColumns.delivery_challan_number && <td className="customer-name-link">{q.delivery_challan_number}</td>}
