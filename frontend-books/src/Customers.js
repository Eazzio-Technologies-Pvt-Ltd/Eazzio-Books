...
function Customers() {
  ...
  const getCondensedMessage = () => {
    const custName = getCustomerName(expandedCustomer);
    const range = getStatementDates();
    const balanceDue = parseFloat(generatedStatement?.balanceDue || 0).toFixed(2);
    const businessName = orgInfo?.name || "our business";
    return `Dear ${custName}, please find your Statement of Accounts from ${range.from} to ${range.to}. Balance Due: ₹${balanceDue}. Thank you. Regards, ${businessName}.`;
  };

  const sendWhatsApp = (e) => {
    if (e && e.stopPropagation) e.stopPropagation();
    if (!generatedStatement) { toast.error('Please generate the statement first.'); return; }
    const phoneVal = expandedCustomer?.mobile || expandedCustomer?.phone || expandedCustomer?.work_phone;
