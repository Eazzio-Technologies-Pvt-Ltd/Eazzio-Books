/**
 * RecurringInvoices.js – Redesigned list view matching Quotes UI
 */
...
const STATUS_COLORS = {
  ...
  stopped: { bg: "#fef2f2", color: "#b91c1c", label: "STOPPED" },
...
function RecurringInvoices() {
  ...
  const [favoriteView, setFavoriteView] = useState(() => localStorage.getItem("favRecurringInvoicesView") || null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [dataRes, customersRes] = await Promise.all([
        apiRequest("/recurring-invoices"),
        apiRequest("/customers"),
      ]);
      setRecurringInvoices(Array.isArray(dataRes?.recurring_invoices) ? dataRes.recurring_invoices : []);
  ...
  useEffect(() => {
    const fav = localStorage.getItem("favRecurringInvoicesView");
    if (fav) {
      setStatusFilter(fav);
    }
  }, []);
  ...
  const filteredData = recurringInvoices.filter((q) => {
    const searchTerms = [
      q.profile_name || "",
      q.reference_number || "",
      q.profile_name || "",
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        selectedIds.map((id) => apiRequest(`/recurring-invoices/${id}`, { method: "DELETE" }))
      );
  ...
  const allViews = [
  { key: "all", label: "All Recurring Invoices" },
      ...
      <div className="full-table-container">
        <div className="full-table-header">
                  ...
                  filteredViews.map(view => {
                    ...
                    return (
                      <div key={view.key} className="view-dropdown-item" onClick={() => { setStatusFilter(view.key); setStatusDropdownOpen(false); }} style={{ background: isSelected ? "#f0f6ff" : "transparent", color: isSelected ? "#006ee6" : "#344054", fontWeight: isSelected ? "500" : "400" }}>
                        ...
                        <span onClick={(e) => { e.stopPropagation(); const newFav = isFav ? null : view.key; setFavoriteView(newFav); if (newFav) { localStorage.setItem("favRecurringInvoicesView", newFav); toast.success(`"${view.label}" set as default view`); } else { localStorage.removeItem("favRecurringInvoicesView"); toast.success("Default view cleared"); } }} style={{ color: isFav ? "#f59e0b" : "#d0d5dd", fontSize: "14px", padding: "2px 6px" }}>{isFav ? "★" : "☆"}</span>
                      </div>
                    );
                  })
                )}
          ...
          <div className="table-actions">
            <button className="btn-new" onClick={() => navigate("/recurring-invoices/new")}>
                ...
                <div className="more-dropdown-menu">
                  <div className="more-dropdown-item">
                    ...
                    <div className="nested-dropdown-menu">
                                            <div className="more-dropdown-item" onClick={() => { setSortBy("profile_name"); setSortOrder("asc"); setMoreMenuOpen(false); }}><span>Profile Name (A-Z)</span></div>
                  ...
                  <div className="more-dropdown-item" onClick={() => { setMoreMenuOpen(false); toast("Import functionality coming soon"); }}>
                    ...
                    <span>Import RecurringInvoices</span>
                  </div>
                  <div className="more-dropdown-item">
                    ...
                    <span>Export RecurringInvoices</span>
                    <svg className="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                    <div className="nested-dropdown-menu">
        ...
        <div className="table-wrapper">
          {loading ? (
            ...
            <table className={`items-table ${clipText ? 'clip-text' : ''}`}>
              ...
              <tbody>
                {sortedData.map((q) => (
                  <tr key={q.id} onClick={() => navigate(`/recurring-invoices/${q.id}`)} style={{ cursor: "pointer", background: selectedIds.includes(q.id) ? "#fcfcfd" : "" }}>
                    ...
                    {visibleColumns.profile_name && <td className="customer-name-link">{q.profile_name}</td>}
                    {visibleColumns.customer_name && <td>{getCustomerName(q.customer_id)}</td>}
                    {visibleColumns.frequency && <td style={{ textTransform: "capitalize" }}>{q.frequency}</td>}
                    {visibleColumns.next_invoice_date && <td>{q.next_invoice_date ? new Date(q.next_invoice_date).toLocaleDateString("en-GB") : "—"}</td>}
                    {visibleColumns.last_invoice_date && <td>{q.last_invoice_date ? new Date(q.last_invoice_date).toLocaleDateString("en-GB") : "—"}</td>}
                    {visibleColumns.end_date && <td>{q.end_date ? new Date(q.end_date).toLocaleDateString("en-GB") : "—"}</td>}
                    {visibleColumns.sales_person && <td>{q.sales_person || "—"}</td>}
