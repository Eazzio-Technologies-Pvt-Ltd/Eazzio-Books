...
function PettyCash() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const filtered = (res?.payments || []).filter(p => p.deposit_to === "Petty Cash");
      ...
      <div className="full-table-container" style={{ background: "#fff", display: "flex", flexDirection: "column", height: "100%", minHeight: "calc(100vh - 60px)" }}>
        <div style={{ padding: "15px 30px", borderBottom: "1px solid #eaeaea", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <h3 style={{ margin: 0, fontWeight: 600, fontSize: "18px" }}>Petty Cash Ledger</h3>
          </div>
          <div style={{ background: "#f0fdf4", color: "#166534", padding: "8px 16px", borderRadius: "8px", fontWeight: "600", border: "1px solid #bbf7d0" }}>
        ...
        <div style={{ padding: "12px 30px", borderBottom: "1px solid #eaeaea", background: "#fdfdfd" }}>
          <div style={{ position: "relative", width: "100%", maxWidth: "400px" }}>
            ...
            <input
              type="text"
              placeholder="Search petty cash transactions..."
              value={search}
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "60px", color: "#667085", background: "#f9fafb", borderRadius: "8px", margin: "24px", border: "1px dashed #d0d5dd" }}>
              <p style={{ margin: "0 0 16px 0", fontSize: "14px" }}>No transactions found for Petty Cash.</p>
            </div>
          ) : (
              ...
              <tbody>
                {filteredPayments.map(payment => (
                  <tr key={payment.id} onClick={() => navigate(`/payments-received/${payment.id}`)} style={{ cursor: "pointer", background: selectedIds.includes(payment.id) ? "#fcfcfd" : "" }}>
                    ...
                    {visibleColumns.amount && <td style={{ textAlign: "right", fontWeight: "600", color: "#10b981" }}>+ ₹{parseFloat(payment.amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
                  </tr>
                ))}
              </tbody>
            </table>
