...
exports.getMonthlyFinanceSummary = async (req, res) => {
  try {
    ...
    const topRecRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM invoices WHERE user_id = $1 AND status != 'Paid' AND invoice_date >= $2` + (tId ? ` AND organization_id = $3` : ""),
      tId ? [userId, fyStartStr, tId] : [userId, fyStartStr]
    );
    const total_receivables = parseFloat(topRecRes.rows[0].total);

    const topPayRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM bills WHERE user_id = $1 AND status != 'Paid' AND is_deleted = false AND bill_date >= $2` + (tId ? ` AND organization_id = $3` : ""),
      tId ? [userId, fyStartStr, tId] : [userId, fyStartStr]
    );
    const total_payables = parseFloat(topPayRes.rows[0].total);

    const topIncRes = await pool.query(
      `SELECT COALESCE(SUM(amount), 0) AS total FROM payments WHERE user_id = $1 AND payment_date >= $2` + (tId ? ` AND organization_id = $3` : ""),
      tId ? [userId, fyStartStr, tId] : [userId, fyStartStr]
    );
    const total_income = parseFloat(topIncRes.rows[0].total);

    const topExpRes = await pool.query(`SELECT COALESCE(SUM(amount), 0) AS total FROM expenses WHERE user_id = $1 AND expense_date >= $2` + (tId ? ` AND organization_id = $3` : ""), tId ? [userId, fyStartStr, tId] : [userId, fyStartStr]);
    const topPmtMadeRes = await pool.query(`SELECT COALESCE(SUM(amount), 0) AS total FROM payments_made WHERE user_id = $1 AND payment_date >= $2` + (tId ? ` AND organization_id = $3` : ""), tId ? [userId, fyStartStr, tId] : [userId, fyStartStr]);
    ...
    const selIncRes = await pool.query(
      `SELECT COALESCE(SUM(amount), 0) AS total FROM payments 
       WHERE user_id = $1 AND EXTRACT(MONTH FROM payment_date) = $2 AND EXTRACT(YEAR FROM payment_date) = $3` + (tId ? ` AND organization_id = $4` : ""),
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    const income_received = parseFloat(selIncRes.rows[0].total);

    const selExpRes = await pool.query(
      `SELECT COALESCE(SUM(amount), 0) AS total FROM expenses 
       WHERE user_id = $1 AND EXTRACT(MONTH FROM expense_date) = $2 AND EXTRACT(YEAR FROM expense_date) = $3` + (tId ? ` AND organization_id = $4` : ""),
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    ...
    const selExpPayRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM bills 
       WHERE user_id = $1 AND status != 'Paid' AND is_deleted = false AND EXTRACT(MONTH FROM due_date) = $2 AND EXTRACT(YEAR FROM due_date) = $3` + (tId ? ` AND organization_id = $4` : ""),
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    ...
    const activeRecurringRes = await pool.query(
      `SELECT amount, frequency, due_day, start_date FROM recurring_expenses 
       ...
       AND start_date <= $2 
       AND (end_date IS NULL OR end_date >= $3)` + (tId ? ` AND organization_id = $4` : ""),
      tId ? [userId, new Date(year, month, 0), new Date(year, month - 1, 1), tId] : [userId, new Date(year, month, 0), new Date(year, month - 1, 1)]
    );
    ...
    const nextProjIncRes = await pool.query(
      ...
      tId ? [userId, nextMonth, nextYear, tId] : [userId, nextMonth, nextYear]
    );
    const next_projected_income = parseFloat(nextProjIncRes.rows[0].total);

    const nextProjExpRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM bills 
       WHERE user_id = $1 AND is_deleted = false AND EXTRACT(MONTH FROM due_date) = $2 AND EXTRACT(YEAR FROM due_date) = $3 AND status != 'Paid'` + (tId ? ` AND organization_id = $4` : ""),
      tId ? [userId, nextMonth, nextYear, tId] : [userId, nextMonth, nextYear]
    );
    ...
    const nextActiveRecurringRes = await pool.query(
      `SELECT amount, frequency, due_day, start_date FROM recurring_expenses 
       ...
       AND start_date <= $2 
       AND (end_date IS NULL OR end_date >= $3)` + (tId ? ` AND organization_id = $4` : ""),
      tId ? [userId, new Date(nextYear, nextMonth, 0), new Date(nextYear, nextMonth - 1, 1), tId] : [userId, new Date(nextYear, nextMonth, 0), new Date(nextYear, nextMonth - 1, 1)]
    );
    ...
    const detRec = await pool.query(
      `SELECT i.invoice_number, c.display_name AS customer_name, i.due_date, i.balance_due, i.status 
       ...
       WHERE i.user_id = $1 AND i.status != 'Paid' AND EXTRACT(MONTH FROM i.due_date) = $2 AND EXTRACT(YEAR FROM i.due_date) = $3` + (tId ? ` AND i.organization_id = $4` : "") + ` ORDER BY i.due_date ASC`,
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    ...
    const detPmtRec = await pool.query(
      `SELECT p.payment_date, c.display_name AS customer_name, i.invoice_number, p.amount, p.payment_mode 
       ...
       WHERE p.user_id = $1 AND EXTRACT(MONTH FROM p.payment_date) = $2 AND EXTRACT(YEAR FROM p.payment_date) = $3` + (tId ? ` AND p.organization_id = $4` : "") + ` ORDER BY p.payment_date DESC`,
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    ...
    const detExp = await pool.query(
      `SELECT e.expense_date, e.category, v.display_name AS vendor_name, e.amount 
       ...
       WHERE e.user_id = $1 AND EXTRACT(MONTH FROM e.expense_date) = $2 AND EXTRACT(YEAR FROM e.expense_date) = $3` + (tId ? ` AND e.organization_id = $4` : "") + ` ORDER BY e.expense_date DESC`,
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    ...
    const detPay = await pool.query(
      `SELECT b.bill_number, v.display_name AS vendor_name, b.due_date, b.balance_due, b.status 
       ...
       WHERE b.user_id = $1 AND b.status != 'Paid' AND b.is_deleted = false AND EXTRACT(MONTH FROM b.due_date) = $2 AND EXTRACT(YEAR FROM b.due_date) = $3` + (tId ? ` AND b.organization_id = $4` : "") + ` ORDER BY b.due_date ASC`,
      tId ? [userId, month, year, tId] : [userId, month, year]
    );
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const incR = await pool.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM payments WHERE user_id = $1 AND EXTRACT(MONTH FROM payment_date) = $2 AND EXTRACT(YEAR FROM payment_date) = $3` + (tId ? ` AND organization_id = $4` : ""),
        tId ? [userId, m, y, tId] : [userId, m, y]
      );
      const expR = await pool.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM expenses WHERE user_id = $1 AND EXTRACT(MONTH FROM expense_date) = $2 AND EXTRACT(YEAR FROM expense_date) = $3` + (tId ? ` AND organization_id = $4` : ""),
        tId ? [userId, m, y, tId] : [userId, m, y]
      );
      const pmtMadeR = await pool.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM payments_made WHERE user_id = $1 AND EXTRACT(MONTH FROM payment_date) = $2 AND EXTRACT(YEAR FROM payment_date) = $3` + (tId ? ` AND organization_id = $4` : ""),
        tId ? [userId, m, y, tId] : [userId, m, y]
      );
    ...
    try {
      const topExpCatRes = await pool.query(
        `SELECT COALESCE(category, 'Other') AS name, COALESCE(SUM(amount), 0) AS value 
         FROM expenses WHERE user_id = $1` + (tId ? ` AND organization_id = $2` : "") + ` GROUP BY category ORDER BY value DESC LIMIT 6`,
    ...
    try {
      const bankRes = await pool.query(
        `SELECT account_name AS name, account_number, current_balance AS balance 
         FROM bank_accounts WHERE user_id = $1 AND is_active = true` + (tId ? ` AND organization_id = $2` : "") + ` ORDER BY current_balance DESC`,
        tId ? [userId, tId] : [userId]
      );
    ...
    res.json({
      top_summary: { total_receivables, total_payables, total_income, total_expenses, net_profit, cash_bank_balance },
      selected_month: { month, year, label, income_received, expenses, staff_salary, writeoff, projected_payments, expected_payables, profit, net_cash_position },
      next_month: { month: nextMonth, year: nextYear, label: nextLabel, projected_income: next_projected_income, projected_payments: next_projected_income, projected_expenses: next_projected_expenses, expected_payables: next_projected_expenses, staff_salary: 0, writeoff: 0, projected_profit: next_projected_profit },
      details: { receivables: detRec.rows, payments_received: detPmtRec.rows, expenses: detExp.rows, payables: detPay.rows, staff_salary: [], writeoffs: [] },
      chartData: { cashFlowYearly, incomeExpense6Months, topExpenses, banks }
    });
